...
class Drone(bs.Actor):
    def __init__(self, spaz):
        ...
        self._drone_material.add_actions(
            conditions=(
                ...
                'or',
                ('they_have_material', self._drone_material),
            ),
    ...
    def fire(self):
        if hasattr(self.grab_node, "position"):
            self.rocket_launcher.shot(
                self.spaz,
                self.x_direction,
                self.z_direction,
                (
                    self.grab_node.position[0],
                    self.grab_node.position[1] - 1,
                    self.grab_node.position[2],
                ),
            )
        ...
        def loop():
            if self.node.exists():
                bs.animate(
                    self._rcombine,
                    'input1',
                    {0: self.node.position[1], 1: self.node.position[1] + 2},
                )
...
class RocketLauncher:
    ...
    def shot(self, spaz, x, z, position) -> None:
        ...
        if time - self.last_shot > 0.6:
            ...
            mag = (
                10.0 / 1
                if babase.Vec3(*direction).length() == 0
                else babase.Vec3(*direction).length()
            )
        ...
        self.ground_material.add_actions(
            ...
            actions=(
                ...
                (
                    'call',
                    'at_connect',
                    babase.Call(self._handle_player_collide),
                ),
            ),
        )
        pos = (0, 0.1, -5)
        self.main_region = bs.newnode(
            'region',
            attrs={
                'position': pos,
                'scale': (30, 0.001, 23),
                'type': 'box',
                'materials': [shared.footing_material, self.ground_material],
            },
        )
