...
class UFO(bs.Actor):
    ...
    def move(self):
        ...
        try:
            ...
            self.target = (
                self.dis[self.r].position[0],
                self.node.position[1],
                self.dis[self.r].position[2],
            )
            bs.animate_array(
                self.node,
                'position',
                3,
                {0: self.node.position, 3.0: self.target},
            )
        except:
            pass
...
class defs:
    ...
    boxes['area_of_interest_bounds'] = (
        (-1.3440, 1.185751251, 3.7326226188)
        + (0.0, 0.0, 0.0)
        + (29.8180273, 15.57249038, 22.93859993)
    )
    boxes['map_bounds'] = (
        (0.0, 2.585751251, 0.4326226188)
        + (0.0, 0.0, 0.0)
        + (29.09506485, 15.81173179, 33.76723155)
    )
...
class DummyMapForGame(bs.Map):
    ...
    def on_preload(cls) -> Any:
        data: Dict[str, Any] = {
            ...
            'bg_mesh_1': bs.getmesh('rampageBG'),
    ...
    def __init__(self) -> None:
        ...
        self.bg1 = bs.newnode(
            'terrain',
            attrs={
                'mesh': self.preloaddata['bg_mesh_1'],
                'lighting': False,
                'background': True,
                'color_texture': self.preloaddata['bg_2'],
            },
        )
        self.bg2 = bs.newnode(
            'terrain',
...
class DummyMapForGame2(bs.Map):
    ...
    def on_preload(cls) -> Any:
        data: Dict[str, Any] = {
            'bg_1': bs.gettexture('menuBG'),
            'bg_2': bs.gettexture('menuBG'),
            'bg_mesh_1': bs.getmesh('thePadBG'),
...
class UFOAttackGame(bs.TeamGameActivity[Player, Team]):
    ...
    scoreconfig = bs.ScoreConfig(
        label='Survived', scoretype=bs.ScoreType.SECONDS, version='B'
    )
    ...
    def __init__(self, settings: dict):
        ...
        if bool(settings["Display Map Area Dimension"]):
            self.game_name = (
                "UFO Attack " + "(" + str(self.col) + "x" + str(self.row) + ")"
            )
        else:
