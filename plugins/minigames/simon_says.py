...
class CustomText(bs.Actor):
    ...
    def __init__(
        ...
        pos = (
            ...
            position[1] + offset[0] + random_offset * (0.5 - random.random()),
            position[2] + offset[0] + random_offset * (0.5 - random.random()),
        )
...
class SimonSays(bs.TeamGameActivity[Player, Team]):
    ...
    def on_begin(self) -> None:
        super().on_begin()
        s = self.settings
        _gameutils.animate_array(
            self.n1, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]}
        )
        _gameutils.animate_array(
            self.n2, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]}
        )
        _gameutils.animate_array(
            self.n3, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]}
        )
        _gameutils.animate_array(
            self.n4, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]}
        )
        _gameutils.animate_array(
            self.n5, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]}
        )
        _gameutils.animate_array(
            self.n6, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]}
        )
        _gameutils.animate_array(
            self.n7, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]}
        )
        _gameutils.animate_array(
            self.n8, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]}
        )
        _gameutils.animate_array(
            self.n9, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]}
        )
    ...
    def spawn_player(self, player: PlayerT) -> bs.Actor:
        ...
        spaz = self.spawn_player_spaz(
            player,
            position=(
                0 + random.uniform(-3.6, 3.6),
                2.9,
                -2 + random.uniform(-3.6, 3.6),
            ),
        )
        assert spaz.node
    ...
    def call_round(self) -> None:
        ...
        if self.numa < 9:
            ...
            if not self.simon:
                line = (
                    false_prefix + "un to the " + self.options[self.numa] + "!"
                )
            else:
        ...
        self.counter_loop = bs.Timer(
            self.speed / 1000, set_counter, repeat=True
        )
    ...
    def check_round(self) -> None:
        ...
        for player in self.players:
            if player.is_alive():
                safe = (
                    True
                    if self.options[self.numa]
                    in self.in_circle(player.actor.node.position_center)
                    else False
                )
                if (self.simon and safe == False) or (
                    (not self.simon) and safe == True
                ):
