...
class ShimlaGame(DeathMatchGame):
    ...
    def create_lift(self, x, y):
        ...
        _tcombine = bs.newnode(
            'combine',
            owner=floor,
            attrs={'input0': x, 'input2': -5.5, 'size': 3},
        )
        mnode = bs.newnode(
            'math', owner=lift, attrs={'input1': (0, 2, 0), 'operation': 'add'}
        )
        _tcombine.connectattr('output', mnode, 'input2')

        _cleaner_combine = bs.newnode(
            'combine',
            owner=cleaner,
            attrs={'input1': 5.6, 'input2': -5.5, 'size': 3},
        )
    ...
    def _handle_lift(self):
        ...
        if lift["state"] == "origin":
            ...
            bs.animate(
                lift["lift"],
                'input1',
                {0: 5.1, 1.3: 5.1, 6: 5 + 12, 9: 5 + 12, 15: 5.1},
            )
            bs.timer(
                16,
                babase.Call(lambda lift: lift.update({'state': 'end'}), lift),
            )
