...
class SettingWindow(bui.Window):
    ...
    def increase_limit(self):
        ...
        try:
            ...
            bui.textwidget(
                edit=self.warn_text, text="Click on number to select it"
            )
    ...
    def decrease_limit(self):
        ...
        try:
            ...
            elif (self.selected == "upper" and Udefault <= 30) or (
                self.selected == "lower" and Ldefault >= -20
            ):
                bui.textwidget(edit=self.warn_text, text="")
            if self.selected == "lower":
                Ldefault -= 1
    ...
    def draw_ui(self):
        ...
        bui.widget(
            edit=self.enable_button,
            up_widget=decrease_button,
            down_widget=self.lower_text,
            left_widget=save_button,
            right_widget=save_button,
        )
        bui.widget(
            edit=save_button,
            up_widget=self.close_button,
            down_widget=self.upper_text,
            left_widget=self.enable_button,
            right_widget=self.enable_button,
        )
        bui.widget(
            edit=self.close_button,
            up_widget=increase_button,
            down_widget=save_button,
            left_widget=self.enable_button,
            right_widget=save_button,
        )
        bui.widget(
            edit=self.lower_text,
            up_widget=self.enable_button,
            down_widget=decrease_button,
            left_widget=self.upper_text,
            right_widget=self.upper_text,
        )
        bui.widget(
            edit=self.upper_text,
            up_widget=save_button,
            down_widget=increase_button,
            left_widget=self.lower_text,
            right_widget=self.lower_text,
        )
        bui.widget(
            edit=decrease_button,
            up_widget=self.lower_text,
            down_widget=self.enable_button,
            left_widget=increase_button,
            right_widget=increase_button,
        )
        bui.widget(
            edit=increase_button,
            up_widget=self.upper_text,
            down_widget=self.close_button,
            left_widget=decrease_button,
            right_widget=decrease_button,
        )
        # --------------------------------------------------------------------------------------------------

        bui.textwidget(
            edit=self.upper_text,
            on_activate_call=babase.Call(self.on_text_click, "upper"),
        )
        bui.textwidget(
            edit=self.lower_text,
            on_activate_call=babase.Call(self.on_text_click, "lower"),
        )
...
class moodlight(babase.Plugin):
    ...
    def _new_init(
        self, vr_overlay_offset: Optional[Sequence[float]] = None
    ) -> None:
        ...
        def changetint():
            if loop:
                Range = (
                    random.randrange(Ldefault, Udefault) / 10,
                    random.randrange(Ldefault, Udefault) / 10,
                    random.randrange(Ldefault, Udefault) / 10,
                )
                bs.animate_array(
                    ...
                    'tint',
                    3,
                    {0.0: gnode.tint, transition_duration: Range},
                )
            else:
                global timer
                timer = None
                bs.animate_array(
                    gnode, "tint", 3, {0.0: gnode.tint, 0.4: default_tint}
                )
