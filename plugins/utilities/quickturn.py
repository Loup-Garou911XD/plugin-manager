...
class Quickturn(babase.Plugin):
    ...
    def wavedash(self) -> None:
        ...
        isMoving = (
            abs(self.node.move_up_down) >= 0.5
        ...
        if t_ms - self.last_wavedash_time_ms >= self._wavedash_cooldown:
            ...
            if turn_power < 0.55:
                turn_power = (
                    8 * turn_power * turn_power * turn_power * turn_power
                )
            else:
            ...
            boost_power = (
                math.sqrt(math.pow(vel[0], 2) + math.pow(vel[1], 2)) * 8
            )
            ...
            args[0].roller_material.add_actions(
                ...
                actions=(
                    (
                        ...
                        'our_node',
                        'at_connect',
                        Quickturn.FootConnectMessage,
                    ),
                    (
                        'message',
                        'our_node',
                        'at_disconnect',
                        Quickturn.FootDisconnectMessage,
                    ),
                ),
            )
