...
class _HostLookupThread(threading.Thread):
    ...
    def __init__(
        self, name: str, port: int, call: Callable[[Optional[str], int], Any]
    ):
        super().__init__()
        self._name = name
        self._port = port
...
def newbuild_favorites_tab(
    ...
    b_height = (
        90
        if uiscale is babase.UIScale.SMALL
        else 142 if uiscale is babase.UIScale.MEDIUM else 130
    )
    b_space_extra = (
        0
        if uiscale is babase.UIScale.SMALL
    ...
    bui.textwidget(
        ...
        position=(
            90 if uiscale is babase.UIScale.SMALL else 120,
            btnv + 120 if uiscale is babase.UIScale.SMALL else btnv + 90,
        ),
        size=(0, 0),
        h_align='center',
        color=(0.8, 0.8, 0.8),
    ...
    bui.buttonwidget(
        parent=self._container,
        size=(30, 30),
        position=(25 if uiscale is babase.UIScale.SMALL else 40, btnv + 10),
        color=(0.6, 0.53, 0.63),
    ...
    self.retry_inter_text = bui.textwidget(
        parent=self._container,
        position=(90 if uiscale is babase.UIScale.SMALL else 120, btnv + 25),
        size=(0, 0),
        h_align='center',
        color=(0.8, 0.8, 0.8),
        v_align='center',
    ...
    bui.buttonwidget(
        parent=self._container,
        size=(30, 30),
        position=(125 if uiscale is babase.UIScale.SMALL else 155, btnv + 10),
        color=(0.6, 0.53, 0.63),
        textcolor=(0.75, 0.7, 0.8),
    ...
    if uiscale is babase.UIScale.SMALL and bui.app.ui_v1.use_toolbars:
        bui.widget(edit=btn1, left_widget=bui.get_special_widget('back_button'))
    btnv -= b_height + b_space_extra
    bui.buttonwidget(
        parent=self._container,
        size=(b_width, b_height),
    ...
    self._columnwidget = bui.columnwidget(
        parent=scrlw, left_border=10, border=2, margin=0, claims_left_right=True
    )
...
def new_on_favorites_connect_press(self) -> None:
    if self._favorite_selected is None:
        ...
        _HostLookupThread(
            name=config['addr'],
            port=config['port'],
            call=bs.WeakCall(self._host_lookup_result),
        ).start()

        if self.retry_inter > 0 and (
            bs.get_connection_to_host_info_2() == {}
            or bs.get_connection_to_host_info_2() == None
            or bs.get_connection_to_host_info_2()['build_number'] == 0
        ):
            bui.screenmessage("Server full or unreachable, Retrying....")
            self._retry_timer = bs.AppTimer(
                self.retry_inter, babase.Call(self._on_favorites_connect_press)
            )
...
def auto_retry_dec(self):
    ...
    if self.retry_inter == 0.0:
        ...
        bui.textwidget(
            edit=self.retry_inter_text, text='%.1f' % self.retry_inter
        )
...
def update(
    ...
    existing_selection: Optional[Selection],
    tab: PublicGatherTab,
) -> None:
...
def on_stats_click(self, widget, party):
    ...
    DisChoices = [
        babase.Lstr(resource="ipp", fallback_value="Connect by IP"),
        babase.Lstr(resource="copy id", fallback_value="Copy Queue ID"),
        babase.Lstr(value="Save"),
    ]
    if party.stats_addr:
    ...
    if (
        ...
        if self._filter_value:
            ...
            self._parties_displayed = {
                ...
                if (filterval in v.name.lower() or filterval in v.address)
                and (
                    v.address not in bannedservers
                    if ENABLE_SERVER_BANNING
                    else True
                )
            }
        else:
...
def replace():
    ...
    manualtab.ManualGatherTab._on_favorites_connect_press = (
        new_on_favorites_connect_press
    )
...
class PartyQuickConnect(bui.Window):
    def __init__(self, address: str, port: int):
        ...
        self.lineup_1_transparent_mesh = bui.getmesh('playerLineup1Transparent')
        ...
        self.move_R = bs.AppTimer(
            0.01, babase.Call(self.move_right), repeat=True
        )

    def move_right(self):
        if self._body_image_target and self._eyes_image:
            bui.buttonwidget(
                edit=self._body_image_target, position=(self.dude_x, 110)
            )
            bui.imagewidget(
                edit=self._eyes_image, position=(self.dude_x + 10, 165)
            )
        else:
            self.move_R = None
    ...
    def connect(self, address, port):
        if not self.closed and (
            ...
            bui.textwidget(
                ...
                text="Retrying....(" + str(self.retry_count) + ")",
            )
            self.retry_count += 1
            bs.connect_to_party(address, port=port)
            self._retry_timer = bs.AppTimer(
                1.5, babase.Call(self.connect, address, port)
            )
