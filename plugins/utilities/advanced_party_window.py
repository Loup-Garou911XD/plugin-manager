        ...
        try:
            ...
            if exc.errno in {
                ...
                babase.print_exception(
                    f'Error on gather ping ' f'(errno={exc.errno})', once=True
                )
        except Exception:
            ...
            try:
                ...
                babase.print_exception(
                    'Error on gather ping cleanup', once=True
                )
...
RecordFilesDir = os.path.join(
    _babase.env()["python_directory_user"], "Configs" + os.sep
)
...
class chatloggThread:
    ...
    def save(self, msg):
        ...
        t = open(
            os.path.join(
                _babase.env()["python_directory_user"], "Chat logged.txt"
            ),
            "a+",
        )
        t.write(x + " : " + msg + "\n")
...
command to kick %s?",
        ...
        try:
            from Language_Packs import (
                ModifiedPartyWindow_LanguagePack as ext_lan_pack,
            )

            if isinstance(ext_lan_pack, dict) and isinstance(
                ext_lan_pack.get(Current_Lang), dict
            ):
    ...
    return (
        Language_Texts.get(text, "#Unknown Text#" if not same_fb else text)
        if not isBaLstr
        else babase.Lstr(
            resource="??Unknown??",
            fallback_value=Language_Texts.get(
                text, "#Unknown Text#" if not same_fb else text
            ),
        )
    )
...
def _get_popup_window_scale() -> float:
    ...
    return (
        ...
        else 1.65 if uiscale is babase.UIScale.MEDIUM else 1.23
    )


def _creat_Lstr_list(string_list: list = []) -> list:
    return [
        babase.Lstr(resource="??Unknown??", fallback_value=item)
...
class ModifiedPartyWindow(bascenev1lib_party.PartyWindow):
    def __init__(self, origin: Sequence[float] = (0, 0)):
        ...
        self._height = (
            365
            if uiscale is babase.UIScale.SMALL
            else 480 if uiscale is babase.UIScale.MEDIUM else 600
        )

        # Custom color here
        self._bg_color = (
            babase.app.config.get("PartyWindow_Main_Color", (0.40, 0.55, 0.20))
            if not isinstance(self._getCustomSets().get("Color"), (list, tuple))
            else self._getCustomSets().get("Color")
        )
        if (
            not isinstance(self._bg_color, (list, tuple))
            or not len(self._bg_color) == 3
        ):
        ...
        bui.Window.__init__(
            ...
            root_widget=bui.containerwidget(
                ...
                scale=(
                    2.0
                    if uiscale is babase.UIScale.SMALL
                    else 1.35 if uiscale is babase.UIScale.MEDIUM else 1.0
                ),
                stack_offset=(
                    (0, -10)
                    if uiscale is babase.UIScale.SMALL
        ...
        self._smoothy_button = bui.buttonwidget(
            parent=self._root_widget,
            scale=0.6,
        ...
        if showMute:
            if msg.split(":")[0].encode('utf-8') in unmuted_names:
                ...
                choices_display.append(
                    _getTransText("mutethisguy", isBaLstr=True)
                )
            else:
                choices.append('unmute')
                choices_display.append(
                    _getTransText("unmutethisguy", isBaLstr=True)
                )
        PopupMenuWindow(
    ...
    def _on_nick_rename_press(self, arg) -> None:
        ...
        okb = bui.buttonwidget(
            ...
            on_activate_call=babase.Call(self._add_nick, arg),
            autoselect=True,
        )
    ...
    def _add_nick(self, arg):
        ...
        if arg:
            ...
            config.commit()
        bui.containerwidget(
            edit=self._nick_rename_window, transition='out_scale'
        )
    ...
    def _reset_game_record(self) -> None:
        try:
            ...
            curFilePath = os.path.join(
                dir_path + os.sep, "__lastReplay.brp"
            ).encode(SystemEncode)
            newFileName = str(
                babase.Lstr(resource="replayNameDefaultText").evaluate()
                + " (%s)"
                % (
            ...
            newFilePath = os.path.join(dir_path + os.sep, newFileName).encode(
                SystemEncode
            )
            ...
            bs.broadcastmessage(
                _getTransText("Game_Record_Saved") % newFileName,
                color=(1, 1, 1),
            )
        except:
            babase.print_exception()
            bs.broadcastmessage(
                babase.Lstr(resource="replayWriteErrorText").evaluate()
                + ""
    ...
    def _on_menu_button_press(self) -> None:
        ...
        DisChoices = [
            (
                _getTransText("unmuteall", isBaLstr=True)
                if is_muted
                else _getTransText("muteall", isBaLstr=True)
            ),
            (
                _getTransText("screenmsgoff", isBaLstr=True)
                if screenmsg
                else _getTransText("screenmsgon", isBaLstr=True)
            ),
            _getTransText("Add_a_Quick_Reply", isBaLstr=True),
            _getTransText("Remove_a_Quick_Reply", isBaLstr=True),
            (
                _getTransText("chatloggeroff", isBaLstr=True)
                if chatlogger
                else _getTransText("chatloggeron", isBaLstr=True)
            ),
        ...
        if self._getCustomSets().get("Enable_HostInfo_Debug", False):
            ...
            DisChoices.append(
                _getTransText("Debug_for_Host_Info", isBaLstr=True)
            )
    ...
    def _on_party_member_press(
        ...
        choices_display = [
            ...
            babase.Lstr(resource="??Unknown??", fallback_value="Info"),
            babase.Lstr(
                resource="??Unknown??",
                fallback_value=_getTransText("Kick_ID") % client_id,
            ),
        ]
        ...
        try:
            if (
                len(
                    self._getCustomSets().get("partyMemberPress_Custom")
                    if isinstance(
                        self._getCustomSets().get("partyMemberPress_Custom"),
                        dict,
                    )
                ...
                choices_display.append(
                    _getTransText("Custom_Action", isBaLstr=True)
                )
        except:
            babase.print_exception()
            ...
            if not os.path.isfile(filePath):
                with open(filePath, "wb") as writer:
                    writer.write(
                        (
                            {
                                "Chinese": u"\xe5\x8e\x89\xe5\xae\xb3\xef\xbc\x8c\xe8\xbf\x98\xe6\x9c\x89\xe8\xbf\x99\xe7\xa7\x8d\xe9\xaa\x9a\xe6\x93\x8d\xe4\xbd\x9c!\
\xe4\xbd\xa0\xe2\x84\xa2\xe8\x83\xbd\xe5\x88\xab\xe6\x89\x93\xe9\x98\x9f\xe5\x8f\x8b\xe5\x90\x97\xef\xbc\x9f\
\xe5\x8f\xaf\xe4\xbb\xa5\xe5\x95\x8a\xe5\xb1\x85\xe7\x84\xb6\xe8\x83\xbd\xe8\xbf\x99\xe4\xb9\x88\xe7\x8e\xa9\xef\xbc\x9f"
                            }.get(Current_Lang, "Thats Amazing !")
                        ).encode("UTF-8")
                    )
            if os.path.getmtime(filePath) != self._caches.get(
                "Vertify_Quickresponse_Text"
            ):
                with open(filePath, "r+", encoding="utf-8") as Reader:
                    ...
                    self._caches["Vertify_Quickresponse_Text"] = (
    ...
    def _write_quick_responds(self, data):
        try:
            with open(
                os.path.join(RecordFilesDir, "Quickmessage.txt"), "wb"
            ) as writer:
    ...
    def _getCustomSets(self):
        try:
            if not hasattr(self, "_caches") or not isinstance(
                self._caches, dict
            ):
                self._caches = {}
            try:
                ...
                ) != self._caches.get("PartyWindow_Sets", {}):
                ...
                try:
                    ...
                    if os.path.isfile(filePath):
                        if os.path.getmtime(filePath) != self._caches.get(
                            "Vertify_MainSettings.json_Text"
                        ):
                            with open(
                                ...
                                self._caches["PartyWindow_Sets"] = json.loads(
                                    Text.decode("utf-8")
                                ).get("Custom_PartyWindow_Sets", {})
                            self._caches["Vertify_MainSettings.json_Text"] = (
                                os.path.getmtime(filePath)
                            )
                except:
                    babase.print_exception()
            return (
                self._caches.get("PartyWindow_Sets")
                if isinstance(self._caches.get("PartyWindow_Sets"), dict)
                else {}
            )
            ...
            if type.startswith("all"):
                ...
                try:
                    if type in ("roster", "fullrecord"):
                        ...
                        if (
                            len(roster["players"]) == 1
                            or type.find("singleplayer") != -1
                        ):
                            if type.find("namefull") != -1:
                            ...
                            if type.find("namefull") != -1:
                                return [
                                    (i["name_full"]) for i in roster["players"]
                                ]
                            elif type.find("name") != -1:
    ...
    def _kick_selected_player(self):
        ...
        if self._popup_party_member_client_id != -1:
            if bs.get_foreground_host_session() is not None:
                ...
                choices = (
                    [0, 30, 60, 120, 300, 600, 900, 1800, 3600, 7200, 99999999]
                    if not (
                        isinstance(
                            self._getCustomSets().get("Ban_Time_List"), list
                        )
                        and all(
                            [
                                isinstance(item, int)
                                for item in self._getCustomSets().get(
                                    "Ban_Time_List"
                                )
                            ]
                        )
                    )
                    else self._getCustomSets().get("Ban_Time_List")
                )
                PopupMenuWindow(
                    ...
                    choices_display=_creat_Lstr_list(
                        [
                            _getTransText("Ban_For_%d_Seconds") % item
                            for item in choices
                        ]
                    ),
                    current_choice="Share_Server_Info",
    ...
    def _update(self) -> None:
        ...
        if (
            roster != self._roster
            or smo_mode != self.smoothy_mode
            or f_chat != self.full_chat_mode
        ):
            self._roster = roster
                    ...
                    for x in range(columns):
                        ...
                        if index < len(self._roster):
                            ...
                            try:
                                if (
                                    self.smoothy_mode == 1
                                    and self._roster[index]['players']
                                ):
    ...
    def popup_menu_selected_choice(
        ...
        if self._popup_type == "banTimePress":
            ...
            if choice == "mute":
                unmuted_names.remove(
                    self.msg_user_selected.split(":")[0].encode('utf-8')
                )
            if choice == "unmute":
                unmuted_names.append(
                    self.msg_user_selected.split(":")[0].encode('utf-8')
                )
            if choice == "copy":
            ...
            if choice == "kick":
                ConfirmWindow(
                    text=_getTransText("Normal_kick_confirm")
                    % self._getObjectByID("account"),
                ...
                self.loading_widget = ConfirmWindow(
                    text="Searching .....",
                    color=self._bg_color,
                    text_scale=1.0,
                    cancel_button=False,
                ...
                ConfirmWindow(
                    text=_getTransText("Admin_Command_Kick_Confirm")
                    % self._getObjectByID("account"),
                ...
                for item in ChoiceDis:
                    NewChoices.append(
                        u"self._edit_text_msg_box('%s','add')"
                ...
                NewChoices.append(
                    u"self._on_nick_rename_press('%s')" % (account)
                )
                p = PopupMenuWindow(
                ...
                customActionSets = (
                    customActionSets.get("partyMemberPress_Custom")
                    if isinstance(
                        customActionSets.get("partyMemberPress_Custom"), dict
                    )
                    else {}
                )
                ChoiceDis = []
            ...
            if choice in ("mute", "unmute"):
                ...
                try:
                    ...
                    bs.broadcastmessage(
                        _getTransText("Something_is_added") % newReply,
                        color=(0, 1, 0),
                    )
            ...
            elif choice in ("hostInfo_Debug",) and isinstance(
                bs.get_connection_to_host_info_2(), dict
            ):
                if bs.get_connection_to_host_info_2() != None:
                    ...
                    NewChoices = [
                        "bs.broadcastmessage(str(bs.get_connection_to_host_info_2().get('%s')))"
                        % ((str(i)).replace("'", r"'").replace('"', r'\\"'))
                        for i in ChoiceDis
                    ]
                    PopupMenuWindow(
                        position=popup_window.root_widget.get_screen_space_center(),
                ...
                ConfirmWindow(
                    text=_getTransText("Restart_Game_Record_Confirm"),
                    action=self._reset_game_record,
                    cancel_button=True,
                    cancel_is_selected=True,
                    color=self._bg_color,
                    text_scale=1.0,
                ...
                if isinstance(pos, int) and isinstance(curKeyWord, str):
                    if curKeyWord in (
                        ...
                        if isinstance(playerName, (list, tuple)):
                            ...
                            self.popup_menu_selected_choice(
                                popup_window,
                                choice.replace(
                                    curKeyWord,
                                    (
                                        playerName.replace("'", r"'").replace(
                                            '"', r'\\"'
                                        )
                                    ),
                                    1,
                            ...
                            bs.broadcastmessage(
                                _getTransText("No_valid_player_found"),
                                (1, 0, 0),
                            )
                            bui.getsound("error").play()
                        ...
                        if (
                            isinstance(playerID, (list, tuple))
                            and isinstance(playerName, (list, tuple))
                            and len(playerName) == len(playerID)
                        ):
                            ChoiceDis = []
                            NewChoices = []
                            ...
                            bs.broadcastmessage(
                                _getTransText("No_valid_player_id_found"),
                                (1, 0, 0),
                            )
                            bui.getsound("error").play()
                        ...
                        self.popup_menu_selected_choice(
                            ...
                            choice.replace(
                                r"{$AccountInfo}",
                                (str(self._getObjectByID("roster")))
                                .replace("'", r"'")
                                .replace('"', r'\\"'),
                                1,
                            ),
            ...
            if len(data) > 0 and choice in data:
                ...
                self._write_quick_responds(data)
                bs.broadcastmessage(
                    _getTransText("Something_is_removed") % choice, (1, 0, 0)
                )
                bui.getsound("shieldDown").play()
            else:
                bs.broadcastmessage(
                    babase.Lstr(resource="errorText"), (1, 0, 0)
                )
                bui.getsound("error").play()
        elif (
            choice.startswith("custom_Exec_Choice_")
            or self._popup_type == "Custom_Exec_Choice"
        ):
            exec(
                choice[len("custom_Exec_Choice_") :]
                if choice.startswith("custom_Exec_Choice_")
                else choice
            )
        else:
...
def fetchAccountInfo(account, loading_widget):
    ...
    try:
        ...
        req = urllib.request.Request(
            url,
            headers={
                "User-Agent": f'BS{_babase.env().get("build_number", 0)}',
...
class CustomAccountViewerWindow(viewer.AccountViewerWindow):
    ...
    def _copy_pb(self):
        ...
        bs.broadcastmessage(
            babase.Lstr(resource='gatherWindow.copyCodeConfirmText')
        )
