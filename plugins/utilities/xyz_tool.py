...
def ShitInit(
    ...
    *,
    ...
    self.offt = bs.newnode(
        'math',
        owner=self.node,
        attrs={'input1': (1.2, 1.8, -0.7), 'operation': 'add'},
    )
    ...
    def update():
        p = self.node.position
        is_moving = (
            abs(self.node.move_up_down) >= 0.01
            or abs(self.node.move_left_right) >= 0.01
        )
        if is_moving:
            ...
            self.txt.text = (
                ...
                + "\nY: "
                + str(round(self.xyz[1], DECIMAL_LIMIT))
                + "\nZ: "
                + str(round(self.xyz[2], DECIMAL_LIMIT))
            )
...
def replaceable_punch(self) -> None:
    ...
    with open(path_aid + '/coords' + c27 + '.txt', 'w') as gg:
        ...
        bui.screenmessage(
            "Coordinates saved in: " + "BombSquad/Saved XYZ/" + "coords" + c27
        )
    ...
    if t_ms - self.last_punch_time_ms >= self._punch_cooldown:
        ...
        if not self.node.hold_node:
            bs.timer(
                0.1,
                bs.WeakCall(
                    self._safe_play_sound, SpazFactory.get().swish_sound, 0.8
                ),
            )
