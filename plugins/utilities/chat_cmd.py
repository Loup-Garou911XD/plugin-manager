        ...
        if len(messages) > 1:
            ...
            if m == px:
                ...
                if n == []:
                    ...
                    cmsg(
                        f' {px}help 2 - for page 2 | all or number of list cmds'
                    )
                    ...
                    if n[1] in [
                        ...
                        if n[2] == '1':
                            cmsg('============== Help 1, Page 1 ==============')
                            cmsg(
                                f' your command prefix is or all commands starts with - {px}'
                            )
                            cmsg(
                                f' {px}list or {px}l --   to see ids of players and execute commands'
                            )
                            cmsg(
                                f' {px}uniqeid or {px}id --   to see accountid/uniqeid of player'
                            )
                            cmsg(
                                f' {px}quit or {px}restart  --  to restart the game'
                            )
                            cmsg(
                                f' {px}mute/unmute  --  to mute chat for everyone in your game'
                            )
                        elif n[2] == '2':
                            cmsg('============== Help 1, Page 2 ==============')
                            cmsg(
                                f' {px}pause  --  to pause everyone in your game'
                            )
                            cmsg(
                                f' {px}nv or {px}night  --  to make night in your game'
                            )
                            cmsg(
                                f' {px}dv or {px}day  --  to make night in your game'
                            )
                            cmsg(
                                f' {px}camera_mode  --  to rotate camera ,repat to off'
                            )
                            cmsg('===========================================')
                elif n[0] == '2':
                    cmsg('============================')
                    ...
                    if n[1] in ['page', 'Page']:
                        if n[2] == '1':
                            cmsg('============== Help 2 Page 1 ==============')
                            cmsg(
                                f' {px}kill all or {px}kill number of list | kills the player'
                            )
                            cmsg(
                                f' {px}heal all or {px}heal number of list | heals the players'
                            )
                            cmsg(
                                f' {px}freeze all or {px}freeze number of list | freeze the player'
                            )
                            cmsg(
                                f' {px}unfreeze/thaw all or {px}unfreeze/thaw number of list | unfreeze the player'
                            )
                            cmsg(
                                f' {px}gloves all or {px}gloves number of list | give gloves to player'
                            )
                            cmsg('============================')
                        elif n[2] == '2':
                            cmsg('============== Help 2 Page 2 ==============')
                            cmsg(
                                f' {px}shield all or {px}shield number of list | give shield the player'
                            )
                            cmsg(
                                f' {px}fall all or {px}fall number of list | teleport in down and fall up the player'
                            )
                            cmsg(
                                f' {px}curse all or {px}curse number of list | curse the player'
                            )
                            cmsg(
                                f' {px}creepy all or {px}creepy number of list | make creepy actor of player'
                            )
                            cmsg(
                                f' {px}inv all or {px}inv number of list | makes invisible player'
                            )
                            cmsg(
                            ...
                            cmsg('============== Help 2 Page 3 ==============')
                            cmsg(
                                f' {px}gm all or {px}gm number of list | give bs gods like powers to player'
                            )
                            cmsg(
                                f' {px}sp all or {px}sp number of list | give superrrrrrr damages when punch to player'
                            )
                            cmsg(
                                f' {px}sleep all or {px}sleep number of list | sleep up the player'
                            )
                            cmsg(
                                f' {px}fly all or {px}fly number of list | fly up the player '
                            )
                    ...
                    cmsg('============================')
                    cmsg(
                        f" {px}d_bomb bombType | changes default bomb | do {px}d_bomb help for bomb names "
                    )
                    cmsg(
                        f' {px}dbc (number of bombs) | changes default count of player'
                    )
                    cmsg('============================')
                ...
                if n == []:
                    ...
                    try:
                        ...
                        cmsg(
                            'Removed'
                        )  # cant use getname() activity alwrady finish
                    except:
                        cmsg('could not found player')
                ...
                if n == []:
                    cmsg(
                        f'Use : {px}sp/superpunch all or {px}sp/superpunch number of list'
                    )
                elif n[0] == 'all':
                    for i in activity_players:
                    ...
                    try:
                        if (
                            not activity_players[
                                int(n[0])
                            ].actor._punch_power_scale
                            == 15
                        ):
                            is_name = session_players[int(n[0])].getname()
                            activity_players[
                            ...
                            cmsg(
                                is_name
                                + ' using super punches get away from him'
                            )
                        else:
                            activity_players[
                                int(n[0])
                ...
                if n == []:
                    cmsg(
                        f'Use: {px}unfreeze/thaw all or {px}unfreeze/thaw number of list'
                    )
                elif n[0] == 'all':
                    for i in activity_players:
                ...
                if n == []:
                    cmsg(
                        f'Use: {px}celebrate all or {px}celebrate number of list'
                    )
                elif n[0] == 'all':
                    for i in activity_players:
                ...
                if n == []:
                    ...
                    try:
                        is_name = session_players[int(n[0])].getname()
                        if (
                            not activity_players[int(n[0])].actor.node.fly
                            == True
                        ):
                ...
                if n == []:
                    ...
                    try:
                        is_name = session_players[int(n[0])].getname()
                        if (
                            not activity_players[int(n[0])].actor.node.hockey
                            == True
                        ):
                            ...
                            activity_players[int(n[0])].actor.node.hockey = (
                                False
                            )
                            activity_players[
                                int(n[0])
                ...
                if n == []:
                    ...
                    cmsg(
                        "bombtypes - ['ice', 'impact', 'land_mine', 'normal', 'sticky','tnt']"
                    )
                elif n[0] in [
                    'ice',
                    'impact',
                ...
                if n == []:
                    ...
                    try:
                        for i in activity_players:
                            ...
                            cmsg(
                                'default bomb count is ' + (str(n[0])) + ' now'
                            )
                    except:
